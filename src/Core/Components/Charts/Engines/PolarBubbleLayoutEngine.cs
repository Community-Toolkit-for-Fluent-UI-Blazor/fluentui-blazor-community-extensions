...
/// <summary>
/// Represents the layout engine responsible for computing the positions of polar scatter data points in a chart.
/// </summary>
internal class PolarBubbleLayoutEngine
{
    /// <summary>
        ...
        CO options)
    {
        ...
        for (var i = 0; i < count; i++)
        {
            ...
            var size = valueRange <= 0 ? 0.0 : (item.Value - minValue) / valueRange;
            var visualRadius = minR + size * (maxR - minR);

            points.Add((i, x, y, visualRadius));
        }
