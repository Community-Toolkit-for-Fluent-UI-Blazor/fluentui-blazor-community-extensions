...
public partial class SchedulerCanvas<TItem> : FluentComponentBase, IAsyncDisposable
{
    ...
    /// <summary>
    /// Represents the relative path to the JavaScript module used by the SchedulerCanvas component.
    /// </summary>
    /// <remarks>This constant is used to reference the JavaScript file required for interop functionality in
    /// the SchedulerCanvas. The path is constructed using the root JavaScript directory defined in
    /// FluentCxConstants.</remarks>
    private const string JavascriptModulePath = FluentCxConstants.JAVASCRIPT_ROOT + "Scheduler/SchedulerCanvas.razor.js";

    /// <summary>
    /// Represents whether the scheduler view has changed since the last render.
    /// </summary>
    ...
    public SchedulerCanvas(LibraryConfiguration configuration)
        : base(configuration)
    {
        Id = Identifier.NewId();
    }
    ...
    /// control.</remarks>
    private float PixelsPerMinute => View == SchedulerView.Day ? Parent!.DaySlotHeight / (60.0f / Parent.DaySubdivisions) : 1;
    ...
    /// <summary>
    /// Gets or sets the current view mode for the scheduler component.
    /// </summary>
    /// <remarks>Use this property to specify how the scheduler displays its data, such as day, week, or month
    /// view. Changing this property updates the scheduler's layout accordingly.</remarks>
    [Parameter]
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        ...
        if (firstRender)
        {
            ...
            await _module.InvokeVoidAsync("FluentUI.Blazor.Community.Scheduler.InitDrop", Id, ContentSelector);
            await _module.InvokeVoidAsync("FluentUI.Blazor.Community.Scheduler.ObserveResize", Id, ContentSelector, _dotNetRef);
            return;
        }

        if ((_hasItemsChanged || _force) && JSModule.Imported)
        {
    ...
    private async Task MeasureLayoutAsync(SchedulerView view)
    {
        ...
        _layout = await _module.InvokeAsync<MeasureLayout>("FluentUI.Blazor.Community.Scheduler.MeasureLayout", Id, null, view);
    }
    ...
    private async Task OnDragStartAsync(SchedulerItem<TItem> item)
    {
        if (_isDragging)
        {
            return;
        ...
        {
            await _module.InvokeVoidAsync("FluentUI.Blazor.Community.Scheduler.StartPointerTracking", _dotNetRef, item.Id, 0);
        }
    ...
    private async Task OnDragEndedAsync(SchedulerItem<TItem> item)
    {
        ...
        if (_module is not null)
        {
            await _module.InvokeVoidAsync("FluentUI.Blazor.Community.Scheduler.StopPointerTracking");
        }
    ...
    private async Task OnResizeStartedAsync(SchedulerResizeEventArgs<TItem> e)
    {
        ...
        if (_module is not null)
        {
            await _module.InvokeVoidAsync("FluentUI.Blazor.Community.Scheduler.StartPointerTracking", _dotNetRef, e.Item.Id, 1);
        }
    ...
    public override async ValueTask DisposeAsync()
    {
        ...
        if (_module != null)
        {
            try
            {
                await _module.InvokeVoidAsync("FluentUI.Blazor.Community.Scheduler.DisposeDrop", Id);
                await _module.InvokeVoidAsync("FluentUI.Blazor.Community.Scheduler.DisposeObserve");
    ...
    {
        _hasItemsChanged = parameters.TryGetValue<List<SchedulerItem<TItem>>>(nameof(Items), out var items) && !Enumerable.SequenceEqual(items, Items);
    ...
    /// <returns>A task that represents the asynchronous operation.</returns>
    private async Task OnDoubleClickItemAsync(SchedulerItem<TItem> item)
    {
    ...
    private async Task OnMonthDropAsync(DragEventArgs e)
    {
        ...
        var rect = await _module.InvokeAsync<RectangleF>("FluentUI.Blazor.Community.Scheduler.GetMonthGridRect", Id);
    ...
    private async Task OnWeekDropAsync(DragEventArgs e)
    {
        ...
        var rect = await _module.InvokeAsync<RectangleF>("FluentUI.Blazor.Community.Scheduler.GetBoundingClientRect", Id);
    ...
    private async Task OnTimelineDropAsync(DragEventArgs e)
    {
        ...
        var rect = await _module.InvokeAsync<RectangleF>("FluentUI.Blazor.Community.Scheduler.GetTimelineRect", Id);
        var scrollLeft = await _module.InvokeAsync<float>("FluentUI.Blazor.Community.Scheduler.GetScrollLeft", Id);
    ...
    /// <summary>
    /// Handles content resize events and updates the layout and items asynchronously.
    /// </summary>
    /// <remarks>This method is intended to be invoked from JavaScript when the content area is resized. It
    /// ensures that the layout and items are repositioned to reflect the new content size.</remarks>
    /// <returns>A task that represents the asynchronous operation.</returns>
    [JSInvokable("Scheduler_OnContentResize")]
    public async Task OnContentResizeAsync()
    {
        await PositionLayoutAndItemsAsync();
    }
