...
The `fluentui-blazor-community-extensions` repository uses Git as its source control system. If you haven't already installed it, you can download it [here](https://git-scm.com/downloads) or if you prefer a GUI-based approach, try [GitHub Desktop](https://desktop.github.com/).
...
On Windows, we prefere to develop using [Visual Studio 2026](https://visualstudio.microsoft.com/downloads/).
...
Now that your machine is setup, you can clone the `fluentui-blazor-community-extensions` repository. Open a terminal and run this command:

```shell
git clone https://github.com/Community-Toolkit-for-Fluent-UI-Blazor/fluentui-blazor-community-extensions.git
```

Cloning via SSH:

```shell
git clone git@github.com:Community-Toolkit-for-Fluent-UI-Blazor/fluentui-blazor-community-extensions.git
```
...
As subject matter experts, *code owners* approve pull requests on the packages they own. There is a required minimum of one code owner for each package. *Code owners* are listed in [CODEOWNERS](https://github.com/Community-Toolkit-for-Fluent-UI-Blazor/fluentui-blazor-community-extensions/blob/main/.github/CODEOWNERS).
