...
/// <summary>
/// Represents a chart palette provider that generates a summer-themed color palette for chart components.
/// </summary>
internal sealed class SummerPaletteProvider : IChartPaletteProvider
{
    /// <summary>
    ...
    public Task<ChartPalette> GenerateAsync(ChartPaletteRequest request)
    {
        ...
        for (var i = 0; i < BaseColors.Length; i++)
        {
            ...
            for (var v = 0; v < variants; v++)
            {
                ...
                var factor = (v - (variants - 1) / 2.0) * 0.05;
