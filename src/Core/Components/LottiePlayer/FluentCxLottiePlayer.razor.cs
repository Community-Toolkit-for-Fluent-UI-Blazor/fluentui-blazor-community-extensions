    ...
    : FluentComponentBase
{
    ...
    /// <summary>
    /// Represents the relative path to the JavaScript module required for the Lottie player component in the FluentCx
    /// library.
    /// </summary>
    /// <remarks>This constant should be used when referencing the JavaScript file necessary for enabling
    /// Lottie animation functionality within FluentCx components. Ensure that the file exists at the specified path to
    /// avoid runtime errors when loading animations.</remarks>
    private const string JavascriptModulePath = FluentCxConstants.JAVASCRIPT_ROOT + "LottiePlayer/FluentCxLottiePlayer.razor.js";
    ...
    public FluentCxLottiePlayer(LibraryConfiguration configuration)
        : base(configuration)
    {
        Id = Identifier.NewId();
    ...
    private async Task LoadAnimationAsync()
    {
        ...
        {
            await _jsInstance.InvokeVoidAsync("FluentUI.Blazor.Community.LottiePlayer.Load", Id, _dotNetRef, Source, Loop, Autoplay, Speed, Renderer);
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        if (firstRender)
        {
            _jsInstance = await JSModule.ImportJavaScriptModuleAsync(JavascriptModulePath);
            await LoadAnimationAsync();
        }
    }
    ...
    public async ValueTask PlayAsync()
    {
        if (_jsInstance is not null)
        {
            await _jsInstance.InvokeVoidAsync("FluentUI.Blazor.Community.LottiePlayer.Play", Id);
        }
    }
    ...
    public async ValueTask PauseAsync()
    {
        if (_jsInstance is not null)
        {
            await _jsInstance.InvokeVoidAsync("FluentUI.Blazor.Community.LottiePlayer.Pause", Id);
        }
    }
    ...
    public async ValueTask StopAsync()
    {
        if (_jsInstance is not null)
        {
            await _jsInstance.InvokeVoidAsync("FluentUI.Blazor.Community.LottiePlayer.Stop", Id);
        }
    }
    ...
    public async ValueTask SetSpeedAsync(double speed)
    {
        ...
        {
            Speed = speed;
            await _jsInstance.InvokeVoidAsync("FluentUI.Blazor.Community.LottiePlayer.SetSpeed", Id, speed);
        }
    }
    ...
    public async ValueTask SetDirectionAsync(LottieDirection direction)
    {
        if (_jsInstance is not null)
        {
            await _jsInstance.InvokeVoidAsync("FluentUI.Blazor.Community.LottiePlayer.SetDirection", Id, direction);
        }
    }
    ...
    public async ValueTask PlaySegments(int startSegment, int endSegment, bool forceFlag = false)
    {
        if (_jsInstance is not null)
        {
            await _jsInstance.InvokeVoidAsync("FluentUI.Blazor.Community.LottiePlayer.PlaySegments", Id, startSegment, endSegment, forceFlag);
        }
    }
    ...
    protected override async ValueTask DisposeAsync(IJSObjectReference jsModule)
    {
        try
        {
            ...
            {
                await jsModule.InvokeVoidAsync("FluentUI.Blazor.Community.LottiePlayer.Dispose", Id);
            }
        }
