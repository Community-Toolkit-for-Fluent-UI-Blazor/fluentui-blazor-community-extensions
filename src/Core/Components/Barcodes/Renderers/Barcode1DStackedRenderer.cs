...
internal sealed class Barcode1DStackedRenderer<TBarcodeOptions>(
    IBarcodeEncoder<Barcode1DStackedPayload, TBarcodeOptions> encoder,
    Func<string> barcodeData,
    TBarcodeOptions barcodeOptions,
    Barcode1DStackedComposer<TBarcodeOptions>? composer = null
    ) : ISurfaceRenderer<BarcodeRenderingOptions> where TBarcodeOptions : IBarcode1DStackedOptions
{
    /// <summary>
        ...
        BarcodeRenderingOptions options)
    {
        ...
        var realComposer = composer ?? new Barcode1DStackedComposer<TBarcodeOptions>();
