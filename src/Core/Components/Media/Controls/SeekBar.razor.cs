...
{
    /// <summary>
    /// Represents the relative path to the JavaScript file used for the SeekBar control.
    /// </summary>
    /// <remarks>This constant is constructed by appending the SeekBar control's JavaScript file path to the
    /// JavaScript root defined in FluentCxConstants. Ensure that the referenced file exists at the specified location
    /// for the SeekBar control to function correctly.</remarks>
    private const string JavascriptFilename = FluentCxConstants.JAVASCRIPT_ROOT + "Media/Controls/SeekBar.razor.js";
    ...
    /// <summary>
    /// Represents a thumbnail value used for displaying a preview image.
    /// </summary>
    private ElementReference _thumbnailElement;

    /// <summary>
    /// Represents the canvas element reference used for rendering thumbnails.
    /// </summary>
    ...
    /// </summary>
    public SeekBar(LibraryConfiguration configuration)
        : base(configuration)
    {
    ...
    /// <summary>
    /// Gets or sets a value indicating whether the seek bar is disabled.
    /// </summary>
    [Parameter]
    public bool Disabled { get; set; }

    /// <summary>
    /// Gets or sets the video element reference associated with the seek bar.
    /// </summary>
    [Parameter]
    public Video? VideoReference { get; set; }
    ...
    /// <summary>
    /// Gets or sets a value indicating whether to display thumbnails during seeking.
    /// </summary>
    [Parameter]
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        if (firstRender)
        {
            ...
            await _module.InvokeVoidAsync("FluentUI.Blazor.Community.SeekBar.Initialize", Id, _dotNetRef);
    ...
    private async Task MeasureWidthAsync()
    {
        ...
        {
            _containerWidth = await _module.InvokeAsync<double>("FluentUI.Blazor.Community.SeekBar.GetWidth", Id);
        }
    }
    ...
    private async Task UpdateSeekFromClientXAsync(double clientX)
    {
        ...
        var offset = await _module.InvokeAsync<double>("FluentUI.Blazor.Community.SeekBar.GetOffsetX", Id, clientX);
    ...
    private async Task SeekToAsync(double time, bool preview = false)
    {
        ...
        if (ShowThumbnails && _module is not null)
        {
            await _module.InvokeVoidAsync("FluentUI.Blazor.Community.SeekBar.GetThumbnail", Id, VideoReference?.Element, _canvasElement, _thumbnailElement);
        }

        var chapter = Chapters.FirstOrDefault(c => c.StartTime.TotalMilliseconds <= CurrentTime && CurrentTime < c.EndTime.TotalMilliseconds);
    ...
    public override async ValueTask DisposeAsync()
    {
        try
        {
            ...
            if (_module is not null)
            {
                await _module.InvokeVoidAsync("FluentUI.Blazor.Community.SeekBar.Dispose", Id);
