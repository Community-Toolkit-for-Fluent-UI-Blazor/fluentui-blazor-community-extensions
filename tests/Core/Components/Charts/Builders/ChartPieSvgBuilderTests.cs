...
public class ChartPieSvgBuilderTests
{
    [Fact]
    public void ChartPieSvgBuilder_Build_RendersSlicesAndLabels()
    {
        var svg = new SvgBuilder();
        var payload = new PiePayloadCollection
        {
        ...
        ChartPieSvgBuilder.Build(svg, payload, themeContext);

        var markup = svg.Build();

        Assert.Contains("chart-pie", markup);
        Assert.Contains("slice-1", markup);
        Assert.Contains("chart-pie-labels", markup);
        Assert.Contains("A", markup);
    }

    [Fact]
    public void ChartPieSvgBuilder_Build_SkipsLabelsWhenDisabled()
    {
        var svg = new SvgBuilder();
        var payload = new PiePayloadCollection
        {
        ...
        Assert.DoesNotContain("chart-pie-labels", markup);
    }
}
