    ...
    : FluentComponentBase
{
    ...
    /// </summary>
    [Parameter]
    public ChatRecorderState State { get; set; } = ChatRecorderState.Idle;
    ...
    /// </summary>
    [Parameter]
    public EventCallback<ChatRecorderState> OnRecorderStateChanged { get; set; }
    ...
    /// <summary>
    /// Starts the audio recording process and updates the state to <see cref="ChatRecorderState.Recording"/>.
    /// </summary>
    /// <returns>Returns a task that represents the asynchronous operation.</returns>
    public async Task StartAsync()
    {
        State = ChatRecorderState.Recording;
    ...
    /// <summary>
    /// Stops the audio recording process and updates the state to <see cref="ChatRecorderState.Processing"/>. This method also invokes the JavaScript function to stop the recording and process the recorded audio data.
    /// </summary>
    /// <returns></returns>
    public async Task StopAsync()
    {
        State = ChatRecorderState.Processing;
    ...
    /// <summary>
    /// Occurs when the audio recording is completed. This method is invoked from JavaScript and updates the state to <see cref="ChatRecorderState.Completed"/>.
    ...
    public async Task RecordingCompleted()
    {
        if (_audioStream is null)
        {
            State = ChatRecorderState.Completed;
            await InvokeAsync(StateHasChanged);
