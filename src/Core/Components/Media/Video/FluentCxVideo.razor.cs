...
public partial class FluentCxVideo : FluentComponentBase
{
    ...
    /// </summary>
    private const string JavaScriptFile = FluentCxConstants.JAVASCRIPT_ROOT + "Media/Video/FluentCxVideo.razor.js";
    ...
    public FluentCxVideo(LibraryConfiguration configuration)
        : base(configuration)
    {
        Id = Identifier.NewId();
    ...
    /// <summary>
    /// Gets or sets the service provider used to resolve dependencies for this component.
    /// </summary>
    [Inject]
    private IServiceProvider ServiceProvider { get; set; } = default!;

    /// <summary>
    /// Gets the video metadata provider used to retrieve metadata for video files.
    /// </summary>
    private IVideoMetadataProvider? MetadataProvider => ServiceProvider.GetService<IVideoMetadataProvider>();
    ...
    private bool IsPreviousDisabled
    {
        get
        {
            ...
            if (_originalPlaylist.Count == 1)
            {
                ...
                {
                    MediaRepeatMode.PlaylistOnce => true,
                    MediaRepeatMode.PlaylistLoop => false,
                    MediaRepeatMode.SingleOnce => true,
                    MediaRepeatMode.SingleLoop => false,
                    _ => true
                };
            }
    ...
    private async Task SetVideoSourceAsync(string? source = null)
    {
        ...
        {
            await _module.InvokeVoidAsync("FluentUI.Blazor.Community.Video.SetSource", Id, src);
            await _module.InvokeVoidAsync("FluentUI.Blazor.Community.Video.SetVolume", Id, _volume);
        }
    }
    ...
    private async Task OnDownloadAsync()
    {
        if (CurrentTrack is not null &&
            _module is not null)
        {
            await _module.InvokeVoidAsync("FluentUI.Blazor.Community.Video.Download", CurrentTrack.Source, CurrentTrack?.Metadata?.Descriptive?.Title ?? CurrentTrack?.Source ?? Path.GetRandomFileName());
        }
    }
    ...
    private async Task PlayAsync()
    {
        if (_module is not null)
        {
            await _module.InvokeVoidAsync("FluentUI.Blazor.Community.Video.Play", Id);
        }
    }
    ...
    private async Task OnPauseAsync()
    {
        if (_module is not null)
        {
            await _module.InvokeVoidAsync("FluentUI.Blazor.Community.Video.Pause", Id);
        }
    }
    ...
    private async Task OnFullScreenAsync()
    {
        if (_module is not null)
        {
            await _module.InvokeVoidAsync("FluentUI.Blazor.Community.Video.Fullscreen", Id);
        }
    }
    ...
    private async Task OnTogglePiPAsync()
    {
        if (_module is not null)
        {
            await _module.InvokeVoidAsync("FluentUI.Blazor.Community.Video.PictureInPicture", Id);
        }
    }
    ...
    private async Task OnTogglePlayPauseAsync()
    {
        if (_module is not null)
        {
            await _module.InvokeVoidAsync("FluentUI.Blazor.Community.Video.TogglePlayPause", Id);
        }
    }
    ...
    private async Task OnPropertiesAsync()
    {
        ...
        await DialogService.ShowDialogAsync<VideoTrackProperties>(options =>
        {
    ...
    private async Task OnChangeVolumeAsync(int value)
    {
        ...
        if (_module is not null)
        {
            await _module.InvokeVoidAsync("FluentUI.Blazor.Community.Video.SetVolume", Id, value);
        }
    }
    ...
    private async Task OnSeekEndAsync(double value)
    {
        if (_module is not null)
        {
            await _module.InvokeVoidAsync("FluentUI.Blazor.Community.Video.Seek", Id, value);
        }
    }
    ...
    private async Task OnStopAsync()
    {
        ...
        if (_module is not null)
        {
            await _module.InvokeVoidAsync("FluentUI.Blazor.Community.Video.Stop", Id);
        }
    ...
    private async Task OnSubtitleClickAsync()
    {
        ...
        {
            Background = VideoState.SubtitleOptions.Background,
            BackgroundColor = VideoState.SubtitleOptions.BackgroundColor,
            SelectedLanguage = VideoState.SubtitleOptions.SelectedLanguage
        };

        var keys = Subtitles.Keys.ToList();

        var result = await DialogService.ShowDrawerAsync<SubtitlePanel>(options =>
        {
            options.Header.Title = Labels.SubtitlesLabel;
            options.Size = DialogSize.Small;
            options.Footer.PrimaryAction.Label = Labels.ApplyLabel;
            options.Footer.SecondaryAction.Label = Labels.CancelLabel;

            options.Parameters.Add(nameof(SubtitlePanel.SubtitleOptions), subtitleOptions);
            options.Parameters.Add(nameof(SubtitlePanel.Content), keys);
            options.Parameters.Add(nameof(SubtitlePanel.Labels), Labels);
        });

        if (!result.Cancelled)
        {
            VideoState.SubtitleOptions = subtitleOptions;
    ...
    private async Task OnPlaybackSpeedClickAsync()
    {
        ...
        var result = await DialogService.ShowDrawerAsync<PlaybackSpeedPanel>(options =>
        {
            options.Header.Title = Labels.PlaybackSpeedLabel;
            options.Footer.PrimaryAction.Label = Labels.ApplyLabel;
            options.Footer.SecondaryAction.Label = Labels.CancelLabel;

            options.Parameters.Add(nameof(PlaybackSpeedPanel.Content), _videoPlaybackSpeedOptions);
            options.Parameters.Add(nameof(PlaybackSpeedPanel.Labels), Labels);
        });

        if (!result.Cancelled && _module is not null)
        {
            await _module.InvokeVoidAsync("FluentUI.Blazor.Community.Video.SetPlaybackRate", Id, _videoPlaybackSpeedOptions.Speed);
        }
    }
    ...
    private async Task OnVideoQualityClickAsync()
    {
        ...
        var result = await DialogService.ShowDrawerAsync<VideoQualityPanel>(options =>
        {
            options.Header.Title = Labels.VideoQualityLabel;
            options.Footer.PrimaryAction.Label = Labels.ApplyLabel;
            options.Footer.SecondaryAction.Label = Labels.CancelLabel;

            options.Parameters.Add(nameof(VideoQualityPanel.Content), CurrentTrack);
            options.Parameters.Add(nameof(VideoQualityPanel.Labels), Labels);
        });
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        ...
        if (firstRender || _hasViewChanged)
        {
            ...
            await _module!.InvokeVoidAsync("FluentUI.Blazor.Community.Video.Dispose", Id);
            await _module!.InvokeVoidAsync("FluentUI.Blazor.Community.Video.Initialize", Id, _compactPlayerVideoReference ?? _videoReference!.Element, _dotNetRef);
    ...
    /// <summary>
    /// Asynchronously retrieves video metadata from the specified URL using the provided name.
    /// </summary>
    ...
    /// cref="VideoMetadata"/> if metadata is successfully retrieved; otherwise, <see langword="null"/>.</returns>
    public async Task<VideoMetadata?> GetMetadataFromUrlAsync(string? url)
    {
        if (MetadataProvider is not null)
        {
            return await MetadataProvider.GetFromUrlAsync(url);
        }
    ...
    /// <summary>
    /// Asynchronously retrieves video metadata from the specified browser file.
    /// </summary>
    ...
    /// <returns>An instance of <see cref="VideoMetadata"/> containing the audio metadata if available; otherwise, null.</returns>
    public async Task<VideoMetadata?> GetAudioMetadataFromFileAsync(IBrowserFile browserFile)
    {
        if (MetadataProvider is not null)
        {
    ...
    /// <returns>A task that represents the asynchronous operation. The task result contains an instance of <see
    /// cref="VideoMetadata"/> with the extracted metadata, or <see langword="null"/> if no metadata provider is
    /// available.</returns>
    public async Task<VideoMetadata?> GetAudioMetadataFromStreamAsync(Stream stream)
    {
        if (MetadataProvider is not null)
        {
            return await MetadataProvider.GetFromStreamAsync(stream);
        }
