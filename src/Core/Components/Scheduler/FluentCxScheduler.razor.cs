...
public partial class FluentCxScheduler<TItem>
{
    ...
    /// <summary>
    /// Initializes a new instance of the FluentCxScheduler class using the specified library configuration.
    /// </summary>
    /// <param name="configuration">The configuration settings to use for initializing the scheduler. Cannot be null.</param>
    public FluentCxScheduler(LibraryConfiguration configuration)
        : base(configuration)
    { }
    ...
    /// <summary>
    /// Gets or sets the data template used to display the item content.
    /// </summary>
    [Parameter]
    ...
    protected async Task LoadItemsAsync()
    {
        ...
        _schedulerSlots = [.. slotBuilder.GetSlots(Localizer, Culture, _startDate, _endDate)];
    ...
    internal string GetViewName(SchedulerView view)
    {
        return view switch
        {
            SchedulerView.Day => Localizer[LanguageResource.CX_Scheduler_DayView],
            SchedulerView.Week => Localizer[LanguageResource.CX_Scheduler_WeekView],
            SchedulerView.Month => Localizer[LanguageResource.CX_Scheduler_MonthView],
            SchedulerView.Year => Localizer[LanguageResource.CX_Scheduler_YearView],
            SchedulerView.Agenda => Localizer[LanguageResource.CX_Scheduler_AgendaView],
            SchedulerView.Timeline => Localizer[LanguageResource.CX_Scheduler_TimelineView],
            _ => view.ToString()
        };
    }
    ...
    {
        var dialog = await DialogService.ShowDialogAsync<SchedulerEventDialog<TItem>>(options =>
        {
            options.Header.Title = item.Id == 0 ? Localizer[LanguageResource.CX_Scheduler_CreateEventTitle] : Localizer[LanguageResource.CX_Scheduler_EditEventTitle];
            options.Footer.PrimaryAction.Label = Localizer[LanguageResource.CX_Scheduler_OK];
            options.Footer.SecondaryAction.Label = Localizer[LanguageResource.CX_Scheduler_Cancel];
            options.Modal = true;
            options.Parameters.Add(nameof(SchedulerEventDialog<>.Content), new SchedulerEventContent<TItem>()
            {
                Culture = Culture,
                Item = item,
        ...
        if (!dialog.Cancelled && dialog.Value is SchedulerItem<TItem> schedulerItem)
        {
    ...
    internal async Task OnDeleteAsync(SchedulerItem<TItem> item)
    {
        ...
        var dialog = await DialogService.ShowConfirmationAsync(
            !hasRecurrence ? Localizer[LanguageResource.CX_Scheduler_DeleteEventMessage] : Localizer[LanguageResource.CX_Scheduler_DeleteRecurrenceMessage],
            !hasRecurrence ? Localizer[LanguageResource.CX_Scheduler_DeleteEventTitle] : Localizer[LanguageResource.CX_Scheduler_DeleteRecurrenceEvent],
            !hasRecurrence ? Localizer[LanguageResource.CX_Scheduler_Yes] : Localizer[LanguageResource.CX_Scheduler_DeleteWholeSeries],
            !hasRecurrence ? Localizer[LanguageResource.CX_Scheduler_No] : Localizer[LanguageResource.CX_Scheduler_DeleteCurrentOccurrence]);
