...
/// <summary>
/// Provides methods for constructing SVG elements that represent an area chart layer, including the area path and its
/// data points.
/// </summary>
/// <remarks>This static class is intended for internal use when rendering area chart visuals within the charting
...
internal static class ChartXYAreaSvgBuilder
{
    /// <summary>
    ...
    /// <param name="payloads">The payloads containing the data and styling information for the area layers.</param>
    /// <param name="context">The theme context providing styling information for the chart.</param>
    public static void Build(
        SvgBuilder svg,
        XYAreaPayloadCollection payloads,
        ...
        ChartThemeContext context)
    {
        ...
        var path = svg.AddPath()
            .WithAttribute("data-id", payload.Id + "-area")
            .WithFill(fill)
            .WithOpacity(opacity)
            .WithStroke("none");
        ...
        path.LineTo(points[^1].X, payload.Baseline);
        path.LineTo(points[0].X, payload.Baseline);
        path.ClosePath();
        ...
        ChartThemeContext context)
    {
        ...
        var path = svg.AddPath()
            .WithAttribute("data-id", payload.Id)
            .WithStroke(stroke)
