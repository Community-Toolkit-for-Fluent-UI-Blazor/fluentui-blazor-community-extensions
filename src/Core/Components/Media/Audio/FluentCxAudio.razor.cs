    ...
    : FluentComponentBase
{
    ...
    /// </summary>
    private DotNetObjectReference<FluentCxAudio>? _dotNetRef;
    ...
    /// </summary>
    private const string JavaScriptFile = FluentCxConstants.JAVASCRIPT_ROOT + "Media/Audio/FluentCxAudio.razor.js";
    ...
    /// <summary>
    /// Represents the height of the audio visualizer container.
    /// </summary>
    private int _height = 280;

    /// <summary>
    /// Represents the width of the audio visualizer container.
    /// </summary>
    ...
    public FluentCxAudio(LibraryConfiguration configuration)
        : base(configuration)
    {
        Id = Identifier.NewId();
    }
    ...
    /// <summary>
    /// Gets a value indicating whether the audio visualizer should be displayed based on the current view and settings.
    /// </summary>
    private bool ViewHasVisualizer => View == AudioPlayerView.Default && ShowVisualizer && !_showPlaylist;
    ...
    private bool IsPreviousDisabled
    {
        get
        {
            ...
            if (_repeatMode == MediaRepeatMode.PlaylistOnce)
            {
                return _currentTrackIndex == 0;
            }

            return _repeatMode == MediaRepeatMode.SingleLoop || _repeatMode == MediaRepeatMode.SingleOnce;
        }
    }
    ...
    /// <summary>
    /// Gets a value indicating whether the playlist should be disabled.
    /// </summary>
    private bool IsPlaylistDisabled => CurrentTrack?.Metadata is null;
    ...
    /// <summary>
    /// Gets the audio metadata provider service from the service provider used to retrieve audio metadata from various sources,
    ///     such as URLs or uploaded files.
    /// </summary>
    private IAudioMetadataProvider? MetadataProvider => Services.GetService<IAudioMetadataProvider>();
    ...
    private async Task OnDownloadAsync()
    {
        if (CurrentTrack is not null &&
            _module is not null)
        {
            await _module.InvokeVoidAsync("FluentUI.Blazor.Community.Audio.Download", CurrentTrack.Source, CurrentTrack?.Metadata?.Descriptive?.Title);
        }
    }
    ...
    private async Task SetAudioSourceAsync()
    {
        ...
        {
            _duration = await _module.InvokeAsync<double>("FluentUI.Blazor.Community.Audio.SetAudioSource", _audioReference!.Element, CurrentTrack.Source);
            await _module.InvokeVoidAsync("FluentUI.Blazor.Community.Audio.SetVolume", _audioReference!.Element, _volume);
        }
    }
    ...
    private async Task PlayAsync()
    {
        if (_module is not null)
        {
            await _module.InvokeVoidAsync("FluentUI.Blazor.Community.Audio.Play", _audioReference!.Element);
        }
    }
    ...
    private async Task OnPauseAsync()
    {
        if (_module is not null)
        {
            await _module.InvokeVoidAsync("FluentUI.Blazor.Community.Audio.Pause", _audioReference!.Element);
        }
    }
    ...
    private async Task OnTogglePlayPauseAsync()
    {
        if (_module is not null)
        {
            await _module.InvokeVoidAsync("FluentUI.Blazor.Community.Audio.TogglePlayPause", _audioReference!.Element);
        }
    }
    ...
    private async Task OnPreviousTrackAsync()
    {
        ...
        switch (_repeatMode)
        {
            case MediaRepeatMode.SingleOnce:
            case MediaRepeatMode.SingleLoop:

                break;

            case MediaRepeatMode.PlaylistOnce:
                {
            ...
            case MediaRepeatMode.PlaylistLoop:
    ...
    private async Task OnPropertiesAsync()
    {
        ...
        await DialogService.ShowDialogAsync<AudioTrackProperties>(options =>
        {
            options.Header.Title = Labels.PropertiesLabel;
            options.Footer.PrimaryAction.Label = Labels.CloseLabel;
            options.Parameters.Add(nameof(AudioTrackProperties.Content), CurrentTrack.Metadata);
            options.Parameters.Add(nameof(AudioTrackProperties.Labels), Labels);
        });
    }
    ...
    private async Task OnSeekAsync(double e)
    {
        ...
        {
            _currentTime = e;
            await _module.InvokeVoidAsync("FluentUI.Blazor.Community.Audio.Seek", _audioReference!.Element, _currentTime);
        }
    }
    ...
    private async Task OnChangeVolumeAsync(double value)
    {
        ...
        if (_module is not null)
        {
            await _module.InvokeVoidAsync("FluentUI.Blazor.Community.Audio.SetVolume", _audioReference!.Element, value);
        }
    }
    ...
    private async Task OnSeekEndAsync(double value)
    {
        if (_module is not null)
        {
            await _module.InvokeVoidAsync("FluentUI.Blazor.Community.Audio.SeekAndResume", _audioReference!.Element, value);
        }
    }
    ...
    private async Task OnStopAsync()
    {
        ...
        if (_module is not null)
        {
            await _module.InvokeVoidAsync("FluentUI.Blazor.Community.Audio.Stop", _audioReference!.Element);
        }
    ...
    protected override async Task OnAfterRenderAsync(bool firstRender)
    {
        if (firstRender)
        {
            _module ??= await JSModule.ImportJavaScriptModuleAsync(JavaScriptFile);
            _dotNetRef?.Dispose();
        ...
        if (_hasViewChanged)
        {
            ...
            await _module!.InvokeVoidAsync(
                "FluentUI.Blazor.Community.Audio.StopObservingResize",
                $"audio-layout-container-{Id}"
            );
        }

        if (ViewHasVisualizer && _module is not null)
        {
            if (firstRender || _hasViewChanged)
            {

                var exists = await _module.InvokeAsync<bool>("FluentUI.Blazor.Community.Audio.Exists", $"audio-layout-container-{Id}");

                if (exists)
                {
                    await _module.InvokeVoidAsync("FluentUI.Blazor.Community.Audio.ObserveResize", $"audio-layout-container-{Id}", _dotNetRef);
                    var size = await _module.InvokeAsync<Size>("FluentUI.Blazor.Community.Audio.Measure", $"audio-layout-container-{Id}");
                    _height = size.Height;
                    _width = size.Width;
                }
            }
        ...
        if ((firstRender || _hasViewChanged) && _audioReference?.Element is not null)
        {
            ...
            await _module!.InvokeVoidAsync(
                "FluentUI.Blazor.Community.Audio.Dispose",
                _audioReference!.Element
            );

            await _module!.InvokeVoidAsync(
                "FluentUI.Blazor.Community.Audio.Initialize",
                _audioReference!.Element,
                _dotNetRef
            );
    ...
    public override Task SetParametersAsync(ParameterView parameters)
    {
        _hasViewChanged = parameters.HasValueChanged(nameof(View), View) ||
                          parameters.HasValueChanged(nameof(ShowVisualizer), ShowVisualizer);

        return base.SetParametersAsync(parameters);
    }

    /// <summary>
    /// Handles a resize event by updating the component's height.
    /// </summary>
    /// <remarks>This method is intended to be invoked from JavaScript via interop when the component's size
    /// changes. It updates the internal height and triggers a UI refresh to reflect the new dimensions.</remarks>
    /// <param name="width">The new width of the audio visualizer. Must be a non-negative integer.</param>
    /// <param name="height">The new height of the audio visualizer, in pixels. Must be a non-negative integer.</param>
    ...
    /// <summary>
    /// Asynchronously retrieves audio metadata from the specified URL using the provided name.
    /// </summary>
    /// <remarks>If the <see cref="MetadataProvider"/> is not initialized, the method returns <see
    /// langword="null"/> without attempting to retrieve metadata.</remarks>
    /// <param name="name">The name to associate with the retrieved audio metadata.</param>
    /// <param name="url">The URL from which to fetch the audio metadata. This parameter can be null.</param>
    /// <returns>A task that represents the asynchronous operation. The task result contains an instance of <see
    /// cref="AudioMetadata"/> if metadata is successfully retrieved; otherwise, <see langword="null"/>.</returns>
    public async Task<AudioMetadata?> GetAudioMetadataFromUrlAsync(string name, string? url)
    {
        if (MetadataProvider is not null)
        {
            return await MetadataProvider.GetFromUrlAsync(name, url);
        }
    ...
    /// <summary>
    /// Asynchronously retrieves audio metadata from the specified browser file.
    /// </summary>
    /// <remarks>If the <see cref="MetadataProvider"/> is not set, the method will return null without
    /// attempting to retrieve metadata.</remarks>
    /// <param name="browserFile">The browser file from which to extract audio metadata. This file must be a valid audio file.</param>
    /// <returns>An instance of <see cref="AudioMetadata"/> containing the audio metadata if available; otherwise, null.</returns>
    public async Task<AudioMetadata?> GetAudioMetadataFromFileAsync(IBrowserFile browserFile)
    {
        if (MetadataProvider is not null)
        {
            return await MetadataProvider.GetFromFileAsync(browserFile);
        }
    ...
    /// <summary>
    /// Asynchronously retrieves audio metadata from the specified stream using the provided name.
    /// </summary>
    /// <remarks>If the <see cref="MetadataProvider"/> is not set, the method returns <see langword="null"/>,
    /// indicating that metadata extraction cannot be performed.</remarks>
    /// <param name="name">The name associated with the audio stream, used to identify the metadata source.</param>
    /// <param name="stream">The stream containing the audio data from which metadata will be extracted.</param>
    /// <returns>A task that represents the asynchronous operation. The task result contains an instance of <see
    /// cref="AudioMetadata"/> with the extracted metadata, or <see langword="null"/> if no metadata provider is
    /// available.</returns>
    public async Task<AudioMetadata?> GetAudioMetadataFromStreamAsync(string name, Stream stream)
    {
        if (MetadataProvider is not null)
        {
            return await MetadataProvider.GetFromStreamAsync(name, stream);
        }
