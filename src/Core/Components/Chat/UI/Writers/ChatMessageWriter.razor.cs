...
public partial class ChatMessageWriter
{
    ...
    private Icon GetMicroIcon()
    {
        ...
        {
            return _audioRecorder.State == RecorderState.Recording ? MicroOffIcon ?? s_micOff : MicroIcon ?? s_mic;
        }
    ...
    private async Task OnAudioRecordingClickAsync()
    {
        if (_audioRecorder is not null)
        {
            if (_audioRecorder.State == RecorderState.Idle ||
                _audioRecorder.State == RecorderState.Completed)
            {
                await _audioRecorder.StartAsync();
            }
            else if (_audioRecorder.State == RecorderState.Recording)
            {
    ...
    /// </summary>
    /// <remarks>The writer becomes visible when the state is <see cref="RecorderState.Idle"/> or <see
    /// cref="RecorderState.Completed"/>.</remarks>
    /// <param name="state">The new recorder state.</param>
    private void OnRecorderStateChanged(RecorderState state)
    {
        _isWriterVisible = state == RecorderState.Idle || state == RecorderState.Completed;
        StateHasChanged();
    }
}
