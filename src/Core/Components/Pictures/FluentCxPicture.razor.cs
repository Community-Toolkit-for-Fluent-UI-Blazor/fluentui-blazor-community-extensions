...
public partial class FluentCxPicture : FluentComponentBase
{
    ...
    /// </summary>
    private readonly List<ResponsiveImage> _internalResponsiveSources = [];

    /// <summary>
    /// Initializes a new instance of the <see cref="FluentCxPicture"/> class with the specified library configuration.
    /// </summary>
    ...
    public FluentCxPicture(LibraryConfiguration configuration)
        : base(configuration)
    {
        Id = Identifier.NewId();
    }
    ...
    /// </summary>
    [Parameter]
    public PictureLoadingBehavior Loading { get; set; } = PictureLoadingBehavior.Auto;

    /// <summary>
    /// Gets or sets the fetch priority of the image.
    /// </summary>
    [Parameter]
    public PictureFetchPriority FetchPriority { get; set; } = PictureFetchPriority.Auto;
    ...
    /// </summary>
    [Parameter]
    public PictureBorderRadius BorderRadius { get; set; } = PictureBorderRadius.None;

    /// <summary>
    /// Gets or sets the custom border radius value when <see cref="BorderRadius"/> is set to <see cref="PictureBorderRadius.Custom"/>.
    /// </summary>
    [Parameter]
    ...
    /// </summary>
    [Parameter]
    public PictureBorderStyle BorderStyle { get; set; } = PictureBorderStyle.None;
    ...
    /// </summary>
    [Parameter]
    public PictureObjectFit ObjectFit { get; set; } = PictureObjectFit.Cover;
    ...
    private string? InternalStyle => new StyleBuilder(Style)
        ...
        .AddStyle("border", $"{BorderWidth} {BorderStyle.ToAttributeValue()} {BorderColor}", BorderStyle != PictureBorderStyle.None)
        .AddStyle("border-radius", BorderRadius == PictureBorderRadius.Circle ? "50%" :
                                   BorderRadius == PictureBorderRadius.RoundSquare ? "10px" :
                                   BorderRadius == PictureBorderRadius.Custom && !string.IsNullOrEmpty(BorderRadiusValue) ? BorderRadiusValue : null, BorderRadius != PictureBorderRadius.None)
        .AddStyle("box-shadow", Shadow?.ToCss(), Shadow is not null)
        .AddStyle("filter", Effects, !string.IsNullOrEmpty(Effects))
        .AddStyle("object-fit", ObjectFit.ToAttributeValue(), ObjectFit != PictureObjectFit.None)
        .Build();
