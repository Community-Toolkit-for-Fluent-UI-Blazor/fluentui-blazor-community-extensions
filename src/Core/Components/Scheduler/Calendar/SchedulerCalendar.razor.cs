...
/// cref="DisabledDateFunc"/> can be used to disable specific dates from selection. The <see cref="OnDateSelected"/> event
...
public partial class SchedulerCalendar
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="SchedulerCalendar"/> component with the specified library configuration.
    /// </summary>
    /// <param name="configuration">Configuration settings for the component, typically provided by dependency injection.</param>
    public SchedulerCalendar(LibraryConfiguration configuration)
        : base(configuration)
    { }
    ...
    {
        if (parameters.TryGetValue<DateTime>(nameof(Month), out var month) && month != Month)
        {
            InitializeOpenPopoverArray();
        }
