    ...
    : IDocumentPayloadBuilder<TPayload>
{
        ...
        SurfaceExportOptions options)
    {
        ...
        {
            WriteGrid(sb, payload.Grid, GridLayerOrder.Background, width, height);
        }

        if (options.IncludeAxes)
        {
            WriteAxes(sb, payload.Axes, AxesLayerOrder.Background, width, height);
        }
        ...
        if (options.IncludeGrid)
        {
            WriteGrid(sb, payload.Grid, GridLayerOrder.Foreground, width, height);
        }

        if (options.IncludeAxes)
        {
            WriteAxes(sb, payload.Axes, AxesLayerOrder.Foreground, width, height);
        }
