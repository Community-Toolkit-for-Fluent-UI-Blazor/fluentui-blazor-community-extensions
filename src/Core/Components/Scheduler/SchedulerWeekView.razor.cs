...
/// </summary>
/// <remarks>The template receives a <see cref="SchedulerSlot"/> instance representing the slot to be rendered.
/// Use this property to customize the appearance and content of individual slots within the scheduler.</remarks>
public partial class SchedulerWeekView
{
    /// <summary>
    /// Initializes a new instance of the SchedulerWeekView class using the specified library configuration.
    /// </summary>
    /// <param name="configuration">The configuration settings used to initialize the scheduler week view. Cannot be null.</param>
    public SchedulerWeekView(LibraryConfiguration configuration)
        : base(configuration)
    {
    }
    ...
    /// </summary>
    /// <remarks>If not set, the property defaults to the current culture of the application. Setting this
    /// property allows customization of date, number, and string formatting to match specific cultural
    /// conventions.</remarks>
    [Parameter]
    public CultureInfo Culture { get; set; } = CultureInfo.CurrentCulture;

    /// <summary>
    /// Gets or sets the collection of dates representing weekdays to be used by the component.
    /// </summary>
    /// <remarks>The list should contain only dates corresponding to weekdays (Monday through Friday).
    /// Modifying this property updates the set of weekdays recognized by the component. The property is typically set
    /// by the parent component when rendering.</remarks>
    [Parameter]
    public IReadOnlyList<DateTime> WeekDays { get; set; } = [];
    ...
    /// <summary>
    /// Gets the computed style string used internally for rendering the component.
    /// </summary>
    private string? InternalStyle => new StyleBuilder(Style)
       .AddStyle("--slot-height", $"{SlotHeight}px", SlotHeight > 0)
        .AddStyle("--number-of-rows", (EndHour - StartHour).ToString(Culture))
        .Build();
