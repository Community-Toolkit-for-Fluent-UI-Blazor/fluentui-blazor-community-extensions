...
public partial class SchedulerAgendaMenu<TItem> : IDisposable
{
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="SchedulerAgendaMenu{TItem}"/> class using the specified library configuration.
    /// </summary>
    /// <param name="configuration">The configuration settings to be used for initializing the component. Cannot be null.</param>
    public SchedulerAgendaMenu(LibraryConfiguration configuration)
        : base(configuration)
    {
    }
    ...
    protected override void OnInitialized()
    {
        ...
        _numberOfDays = Parent.NumberOfDays.ToString(Parent.Culture);
    }
    ...
    {
        if (int.TryParse(_numberOfDays, NumberStyles.Integer, Parent!.Culture, out var parsedNumberOfDays) &&
            parsedNumberOfDays > 1 && parsedNumberOfDays <= 21)
        {
            Parent!.SetAgendaDays(parsedNumberOfDays);
        }
        else
        {
            Parent!.SetAgendaDays(14);
        }
