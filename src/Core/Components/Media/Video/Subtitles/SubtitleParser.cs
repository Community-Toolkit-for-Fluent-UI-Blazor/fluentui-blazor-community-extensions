...
public partial class SubtitleParser(SubtitleParserOptions options)
{
        ...
        while (true)
        {
            ...
            else
            {
                ...
                else if (buffer.Count == 1)
                {
                    ...
                    var match = TimeCodeRegex().Match(rawLine);
                    if (match.Success)
                    {
    ...
    /// <summary>
    /// Determines whether the specified line consists entirely of white-space characters.
    /// </summary>
    /// <remarks>This method iterates through each character in the line to check for white-space characters.
    /// It is useful for validating input or processing text data.</remarks>
    /// <param name="line">The line to evaluate, represented as a read-only memory of characters. An empty line is considered white space.</param>
    /// <returns>true if the line is empty or contains only white-space characters; otherwise, false.</returns>
    private static bool IsWhiteSpace(ReadOnlyMemory<char> line)
    {
        if (line.Length == 0)
        {
            return true;
    ...
    private SubtitleEntry BuildEntry(List<ReadOnlyMemory<char>> buffer, double start, double end)
    {
    ...
    private string NormalizeText(string text)
    {
        return options.Normalization switch
        {
            ...
            TextNormalization.Sentence => text.Length > 0 ? char.ToUpper(text[0]) + text[1..].ToLowerInvariant() : text,
            _ => text
        };
    }
