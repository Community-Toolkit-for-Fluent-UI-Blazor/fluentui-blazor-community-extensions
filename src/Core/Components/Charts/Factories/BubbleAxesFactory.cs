...
internal sealed class BubbleAxesFactory : IAxisFactory<CategoryLineOptions>
{
    public (ChartAxis XAxis, ChartAxis YAxis) CreateAxes(
        ...
        ChartRect plotArea)
    {
        var bubbleSeries = series
            .Where(s => s.IsVisible && s.ChartType == ChartType.Bubble)
