...
internal sealed class StepAxisFactory : IAxisFactory<CategoryLineOptions>
{
    public (ChartAxis XAxis, ChartAxis YAxis) CreateAxes(
        ...
        ChartRect plotArea)
    {
        var stepSeries = series
            .Where(s => s.IsVisible &&
                        (s.ChartType == ChartType.Step))
            .OfType<Series.LineSerie>()
            .ToList();

        if (stepSeries.Count == 0)
        {
            return (
